...
| DOA-RRA-007 | Slim read data | Move active slim command-catalog fixture paths from `/fsx/control_data/genomic_data/organism_reads_slim` to `/fsx/references/genomic_data/organism_reads_slim`. | SUCCESS | plan_amendment | Gate 3 | Agent D | Updated DayOA `.test_data` fixtures and catalog docs from the control-data slim root to the references slim root; `python -m pytest -q tests/test_workflow_catalog.py` in the DAY-EC environment returned `9 passed`. |  | DayOA command-catalog fixtures consume slim validation reads through the single references DRA. |
