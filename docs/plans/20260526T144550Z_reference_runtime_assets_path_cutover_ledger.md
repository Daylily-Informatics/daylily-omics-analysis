...
| DOA-RRA-005 | Release | Create an annotated DayOA tag after path-cutover tests pass; exact future tag must be selected at execution time. | OPEN | feature_implementation | Gate 3 | Agent D | User explicitly requested including all DayOA changes, including pre-existing MultiQC env/doc/plan changes not made by this agent, in the cutover release. |  | Pending commit and annotated tag `1.0.37`. |
