...
| A6 | `agent-release` | Commit/tag DayOA and update/tag DYEC pin if required. | IN_PROGRESS | DayOA patch train reached `10.0.12`; DYEC pin/self release reached `10.0.21`. The latest patch fixes the active `produce_samtools_metrics` gather marker exposed by the HiOMR kitchen sink run. |
| A7 | `agent-headnode` | Prepare dyecX4/profile `lsmc` runs from new DayOA tag via persistent `ubuntu` tmux and `dy-r`. | IN_PROGRESS | dyecX4 headnode has DYEC `10.0.21` available locally; Sentieon runtime `/fsx/references/runtime_assets/cached_envs/sentieon-genomics-202503.03` verified. Workflow launches use `dyec workflow launch`, whose controller runs `dy-r` in persistent `ubuntu` tmux sessions. |
| A8 | `agent-live-runs` | Dry-run then live-run HG003 acceptance analyses. | IN_PROGRESS | Final acceptance set uses DayOA `10.0.10` for current/prior ILMN pangenome, ILMN hg38 solo, and ONT hg38_broad solo, plus DayOA `10.0.12` for the HiOMR kitchen sink after the QC/gather marker fixes. All matching dry-runs passed; live runs are non-terminal as of `2026-06-12T17:05Z`. |
