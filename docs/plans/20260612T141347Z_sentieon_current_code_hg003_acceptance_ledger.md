...
| A5 | `agent-tests` | Run slim local tests and static checks. | COMPLETE | Current focused suite: `python -m pytest tests/test_sentieon_model_bundle_config.py tests/test_ont_fastq_contracts.py tests/test_snakemake_parser_contracts.py tests/test_complete_genomics_sentieon.py tests/test_htd_callers_contract.py tests/test_multiqc_qc_targets.py -q` -> 65 passed. `bash -n bin/dayoa_sentieon_cli`, `python -m py_compile workflow/scripts/parse-vcfeval-summary.py`, and `git diff --check` passed. |
| A6 | `agent-release` | Commit/tag DayOA and update/tag DYEC pin if required. | IN_PROGRESS | DayOA patch train reached `10.0.16`; DYEC pin/self release reached `10.0.25`. DayOA `10.0.16` fixes the HiOMR target-wrapper marker bugs and preserves the forward-only 350GB future `sentieon_bwa_sort` memory request from `10.0.15`. |
| A7 | `agent-headnode` | Prepare dyecX4/profile `lsmc` runs from new DayOA tag via persistent `ubuntu` tmux and `dy-r`. | IN_PROGRESS | dyecX4 launches now use DYEC `10.0.25`; Sentieon runtime `/fsx/references/runtime_assets/cached_envs/sentieon-genomics-202503.03` verified. Workflow launches use `dyec workflow launch`, whose controller runs `dy-r` in persistent `ubuntu` tmux sessions. |
| A8 | `agent-live-runs` | Dry-run then live-run HG003 acceptance analyses. | IN_PROGRESS | Current/prior ILMN pangenome and ONT solo are terminal `exit_code=0`; replacement ILMN hg38 solo `sentup_hg003_ilmn30x_hg38_solo_20260612T181511Z` remains non-terminal and progressing. HiOMR DayOA `10.0.16` dry-run `sentup_hg003_hiomr_kitchensink_20260612T184900Z_dryrun` passed, and live replacement `sentup_hg003_hiomr_kitchensink_20260612T185650Z` has launched with DRA export-on-success. |
| A9 | `agent-evidence` | DRA export and acceptance-report inputs. | IN_PROGRESS | DRA export receipts are successful for ONT solo, current ILMN pangenome, and prior ILMN pangenome. ILMN hg38 solo and replacement HiOMR kitchen sink exports are pending terminal success. |
...
- `2026-06-12T18:45Z`: read-only poll showed ILMN solo progressing without errors and latest HiOMR kitchen-sink run progressing under `-k` but with real Snakemake error markers in target aliases `produce_expansionhunter` and `produce_relatedness`. Inspection showed upstream gather outputs were created and the failures were target-wrapper contract bugs: the aliases had input/log/benchmark declarations but no done-marker output or shell block.
- `2026-06-12T18:47Z`: patched `workflow/rules/expansionhunter.smk` and `workflow/rules/relatedness_batch.smk` so `produce_expansion_hunter`, `produce_expansionhunter`, `produce_expansionhunter_multiqc`, and `produce_relatedness` emit explicit done markers and touch logs. Added regression coverage in `tests/test_multiqc_qc_targets.py`. Focused checks passed: `python -m pytest tests/test_sentieon_model_bundle_config.py tests/test_ont_fastq_contracts.py tests/test_snakemake_parser_contracts.py tests/test_complete_genomics_sentieon.py tests/test_htd_callers_contract.py tests/test_multiqc_qc_targets.py -q` -> 65 passed; `bash -n bin/dayoa_sentieon_cli`, `python -m py_compile workflow/scripts/parse-vcfeval-summary.py`, and `git diff --check` passed.
- `2026-06-12T18:56Z`: DayOA `10.0.16` HiOMR replacement dry-run `sentup_hg003_hiomr_kitchensink_20260612T184900Z_dryrun` completed with `exit_code=0` and no error markers.
- `2026-06-12T18:57Z`: launched DayOA `10.0.16` HiOMR replacement live run `sentup_hg003_hiomr_kitchensink_20260612T185650Z` with DRA export-on-success to `s3://lsmc-ssf-sequencing-data/derived/analysis_results/sentieon-upgrade/ubuntu/sentup_hg003_hiomr_kitchensink_20260612T185650Z/`. The earlier non-terminal HiOMR run `sentup_hg003_hiomr_kitchensink_20260612T175000Z` remains untouched and is not the acceptance gate for the 4NA follow-on rerun.
