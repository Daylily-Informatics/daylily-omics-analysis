...
| DRYRUN-001 | Cluster dry-run | Run DayOA dry-run on `altairval` in a new workset. | OPEN | contract_test | Gate 5 | Agent 5 |  |  | Pending cloneable implementation ref and `ubuntu` tmux dry-run with `dy-r`. |
