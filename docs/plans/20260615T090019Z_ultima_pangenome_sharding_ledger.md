...
| LIVE-001 | Cluster live | Run full sharded HG003 pilot from mounted read-only CRAM/CRAI. | IN_PROGRESS | feature_implementation | Gate 5 | Agent 5 | Live command sent in tmux session `altair_hg003_ultima_pgs_20260615T090753Z` at `2026-06-15T09:09:42Z`; setup reached 21/57 steps; `pre_prep_ultima_cram` job `175` completed far enough for 18 shard jobs `176`-`193` to submit; shard jobs pending for `Priority` with `threads: 128`, `mem_mb=200000`; `squeue --start` estimates first shard at `2026-06-15T16:18:00Z`, later shards into `2026-06-16`, and jobs `192`-`193` as `N/A`. |  | Live run active; sharded caller jobs submitted and waiting for scheduler priority. |
