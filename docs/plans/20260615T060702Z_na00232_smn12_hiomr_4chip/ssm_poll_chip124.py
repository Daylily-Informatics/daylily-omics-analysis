#!/usr/bin/env python3
"""Poll NA00232 chip124 DayOA tmux/log state."""
...
if [[ -f "$log" ]]; then
  ...
  grep -E "__DRYRUN_RC__|__LIVE_RC__|RETURN CODE|Job stats|total" "$log" | tail -n 60 || true
  echo "__LOG_TAIL_BYTES_BEGIN__"
  python3 -c "from pathlib import Path; p=Path('$log'); data=p.read_bytes(); print(data[-5000:].decode('utf-8', errors='replace'))"
...
"""
    result = run_shell(
        ...
        timeout=120,
        comment="Poll NA00232 chip124 DayOA state",
    )
    payload = {
