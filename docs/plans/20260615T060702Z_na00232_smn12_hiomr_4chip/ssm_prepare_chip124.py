#!/usr/bin/env python3
"""Prepare the approved NA00232 chip1+chip2+chip4 DayOA workdir on dyecX4."""
...
git rev-parse HEAD | tee logs/chip124_dayoa_head.txt
git status --short --branch | tee logs/chip124_dayoa_status.txt
wc -l config/samples.tsv config/units.tsv
python3 -c "import csv; row = next(csv.DictReader(open('config/units.tsv'), delimiter='\\t')); print('__UNIT__\\tRUNID\\t' + row['RUNID']); print('__UNIT__\\tLANEID\\t' + row['LANEID']); print('__UNIT__\\tONT_FASTQS\\t' + str(len(row['ONT_R1_PATH'].split(','))))"
"""
    result = run_shell(
        target.instance_id,
        args.region,
        ...
        timeout=600,
        comment="Prepare NA00232 chip124 DayOA workdir",
    )
