#!/usr/bin/env python3
"""Launch full-depth two-sample SMN12 DayOA dry-run in an ubuntu tmux session."""
...
SESSION = "dayoa_smn12_full_chip124_20260615T092300Z_dryrun"
DRYRUN_LOG = "logs/dryrun_smn12_full_chip124_20260615T092300Z.log"
PATCH_LOG = "logs/smn12_runtime_patch_full_chip124_20260615T092300Z.log"
...
def main() -> int:
    ...
    dry_cmd = (
        f"dy-r {TARGETS} {FLAGS} -n 2>&1 | tee {DRYRUN_LOG}; "
        "rc=${PIPESTATUS[0]}; "
        f"echo __DRYRUN_RC__=$rc | tee -a {DRYRUN_LOG}; "
        "exit $rc"
    )
    dry_cmd_b64 = base64.b64encode(dry_cmd.encode("utf-8")).decode("ascii")
    script = f"""
set -euo pipefail
repo="{REMOTE_REPO}"
session="{SESSION}"
...
if [[ -s "$repo/{DRYRUN_LOG}" ]]; then
  echo "__ERROR__	dryrun_log_exists	$repo/{DRYRUN_LOG}"
  exit 22
fi
...
sleep 1
dry_cmd=$(python3 -c "import base64; print(base64.b64decode('{dry_cmd_b64}').decode())")
tmux send-keys -t "$session" "$dry_cmd" C-m
echo "__TMUX_SESSION__	$session"
echo "__REMOTE_REPO__	$repo"
echo "__DRYRUN_LOG__	$repo/{DRYRUN_LOG}"
"""
    result = run_shell(
        target.instance_id,
        args.region,
        ...
        timeout=120,
        comment="Launch full-depth SMN12 chip124 DayOA dry-run tmux",
    )
    ...
    payload = {
        ...
        "targets": TARGETS,
        "flags": FLAGS,
        "dryrun_log": f"{REMOTE_REPO}/{DRYRUN_LOG}",
