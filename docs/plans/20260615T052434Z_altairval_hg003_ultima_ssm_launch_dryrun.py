...
def remote_script(*, analysis_id: str, session: str) -> str:
    ...
    dry_run = "dy-r produce_pangenome_ug_vcf -p -j 150 -k -T 0 --rerun-triggers mtime -n"
    commands = [
        "set -o pipefail",
        ...
        checked(
            f"day-clone -t {q(TAG)} -d {q(analysis_id)} --executing-entity ubuntu --repository daylily-omics-analysis",
            "day_clone",
        ),
    ...
    return textwrap.dedent(
        ...
        mkdir -p "$RUN_DIR"
        printf '\\n__DAYOA_LAUNCH__=%s tag=%s started=%s\\n' "$SESSION" {q(TAG)} "$(date --iso-8601=seconds)" >> "$TMUX_LOG"
...
def main() -> None:
    ...
    try:
        result = run_shell(
            ...
            profile=PROFILE,
            timeout=180,
            comment=f"Launch altairval HG003 Ultima dry-run {args.session}",
        )
