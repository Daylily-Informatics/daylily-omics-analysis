...
def remote_script(*, analysis_id: str, session: str) -> str:
    ...
    live_run = "dy-r produce_pangenome_ug_sharded_vcf produce_snv_concordances -p -j 150 -k -T 0 --rerun-triggers mtime"
    commands = [
        f"cd {q(repo_dir)}",
...
def main() -> None:
    ...
    try:
        result = run_shell(
            ...
            profile=PROFILE,
            timeout=180,
            comment=f"Send live sharded Ultima pangenome commands {args.session}",
        )
