...
def remote_script() -> str:
    ...
    commands = [
        ...
        "echo __DAYOA_STAGE__=monolithic_concordance_live_start $(date --iso-8601=seconds)",
        (
            f"{live_run}; live_rc=$?; "
            "if [[ $live_rc -ne 0 ]]; then echo __DAYOA_FAIL__=monolithic_concordance_live rc=$live_rc $(date --iso-8601=seconds); "
            "else echo __DAYOA_STAGE__=monolithic_concordance_live_done rc=0 $(date --iso-8601=seconds); fi"
        ),
    ]
...
def main() -> None:
    try:
        result = run_shell(
            ...
            profile=PROFILE,
            timeout=180,
            comment="Send monolithic sentpg concordance live run",
        )
