...
WORKDIR_NAME="hg003a_altair3_hiomr_ilmn20x_ont7x_1022"
...
SAMPLES="${MANIFEST_ROOT}/hiomr_ilmn20x_ont7x_samples.tsv"
UNITS="${MANIFEST_ROOT}/hiomr_ilmn20x_ont7x_units.tsv"
...
tmux send-keys -t "${SESSION}" "dy-r produce_alignstats produce_sentdhiomr_snv_vcf produce_snv_concordances --config 'dedupers=[\"dmd\"]' -p -j 234 -k -T 0 --rerun-triggers mtime --max-jobs-per-second 8 > daylily_run_1024.log 2>&1; echo __DS002_RUN1024_RC__:\$? | tee ${RC_FILE}" Enter

{
    echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] DS-002 launch queued in tmux ${SESSION} at tag 1.0.24."
