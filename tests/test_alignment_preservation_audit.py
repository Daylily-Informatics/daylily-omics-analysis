...
def test_alignment_preservation_audit_rule_config_and_targets() -> None:
    ...
    assert 'config["alignment_preservation_audit"]' in rule
    assert 'config.get("alignment_preservation_audit"' not in rule
    assert '.get("threads", 1)' not in rule
    assert '.get("env_yaml", "../envs/samtools_v0.1.yaml")' not in rule
