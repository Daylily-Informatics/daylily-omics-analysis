...
FORMAT_SCRIPT_PATH = REPO_ROOT / "workflow" / "scripts" / "format_multiqc_float_tables.py"
...
def _load_format_module() -> ModuleType:
    spec = importlib.util.spec_from_file_location(
        "format_multiqc_float_tables_under_test", FORMAT_SCRIPT_PATH
    )
    if spec is None or spec.loader is None:
        raise AssertionError(
            f"Unable to import {FORMAT_SCRIPT_PATH.relative_to(REPO_ROOT)}"
        )
    module = importlib.util.module_from_spec(spec)
    sys.modules["format_multiqc_float_tables_under_test"] = module
    spec.loader.exec_module(module)
    return module
...
def test_float_formatter_rewrites_staged_tables_and_writes_overlay(tmp_path: Path) -> None:
    module = _load_format_module()
    table = tmp_path / "contamination_mqc.tsv"
    table.write_text(
        "Sample\tcontamination_fraction\tstatus\nS1\t0.123456\tok\nS2\t1e-3\tok\n",
        encoding="utf-8",
    )
    overlay = tmp_path / "multiqc_float_format_config.yaml"

    changed, columns = module._format_tsv(table)
    module._write_format_config(overlay, {"contamination": columns})

    assert changed is True
    assert columns == ["contamination_fraction"]
    assert "0.1235" in table.read_text(encoding="utf-8")
    assert "0.0010" in table.read_text(encoding="utf-8")
    parsed = yaml.safe_load(overlay.read_text(encoding="utf-8"))
    assert parsed["custom_data"]["contamination"]["headers"][
