...
def _wildcards(lines: list[str]) -> set[str]:
    ...
    for match in WILDCARD_RE.finditer(text):
        name = match.group(1)
        if name in EXCLUDED_WILDCARD_NAMES or name.isupper():
            continue
        names.add(name)
    return names
...
def test_log_and_benchmark_wildcards_match():
    mismatches = []
    for relpath, line, rule_name, lines, directives in _rules():
        log_start, log_end = directives["log"]
        benchmark_start, benchmark_end = directives["benchmark"]
        log_wildcards = _wildcards(lines[log_start:log_end])
        benchmark_wildcards = _wildcards(lines[benchmark_start:benchmark_end])
        if log_wildcards != benchmark_wildcards:
            mismatches.append(
                f"{relpath}:{line}:{rule_name}:"
                f"log={sorted(log_wildcards)}:"
                f"benchmark={sorted(benchmark_wildcards)}"
            )
