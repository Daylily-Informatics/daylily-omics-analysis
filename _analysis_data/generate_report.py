...
def load_concordance(wf_name: str) -> pd.DataFrame:
    ...
    mask = (df["CmpFootprint"] == CONCORDANCE_FOOTPRINT) & (df["SNPClass"].isin(SNP_CLASSES))
    return df.loc[mask, ["Sample", "SNPClass", "Fscore", "Sensitivity-Recall",
                         "Precision", "CmpFootprint", "platform", "workflow",
...
for i, snp_cls in enumerate(SNP_CLASSES):
    ax = axes[i]
    sub = all_conc[all_conc["SNPClass"] == snp_cls]
