...
for wf, meta in WORKFLOWS.items():
    ...
    mask = df["CmpFootprint"].isin(FOOTPRINTS) & df["SNPClass"].isin(SNP_CLASSES)
    sub = df.loc[mask, ["Sample", "SNPClass", "Fscore", "CmpFootprint"]].copy()
...
for row, fp in enumerate(FOOTPRINTS):
    for col, snp_cls in enumerate(SNP_CLASSES):
        ax = axes[row][col]
        sub = merged[(merged["CmpFootprint"] == fp) & (merged["SNPClass"] == snp_cls)]
