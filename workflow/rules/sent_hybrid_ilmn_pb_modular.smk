...
ALIGNERS_DHIPM = ["sentmm2"]

# Base temp directory prefix for intermediate files
def _dhipm_tmp(wildcards):
    return f"{MDIR}{wildcards.sample}/align/{wildcards.alnr}/{wildcards.ddup}/snv/sentdhipm/vcfs/{wildcards.dchrm}/tmp"

# ---------------------------------------------------------------------------
# Rule 1: SR Alignment - Align Illumina FASTQs with sentieon bwa mem
# ---------------------------------------------------------------------------
rule sentdhipm_sr_align:
    ...
    output:
        ...
        bai=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/sr_aligned.bam.bai",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHIPM)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/log/{sample}.{alnr}.{ddup}.{dchrm}.sr_align.log",
    threads: config['sentdhipm']['threads']
    conda:
        "../envs/sentieon_v0.3.yaml"
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhipm.{dchrm}.sr_align.bench.tsv"
    resources:
    ...
    shell:
        ...
        export TMPDIR="/dev/shm/sentdhipm_sr_${{timestamp}}_$$";
...
rule sentdhipm_pass1:
    ...
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHIPM)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/log/{sample}.{alnr}.{ddup}.{dchrm}.pass1.log",
    threads: config['sentdhipm']['threads']
    conda:
        "../envs/sentieon_v0.3.yaml"
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhipm.{dchrm}.pass1.bench.tsv"
    resources:
        partition="i192mem,i192bigmem",
        threads=config['sentdhipm']['threads'],
        vcpu=config['sentdhipm']['threads'],
    ...
    shell:
        ...
        timestamp=$(date +%Y%m%d%H%M%S);
        export TMPDIR="/dev/shm/sentdhipm_p1_${{timestamp}}_$$";
        export SENTIEON_TMPDIR="$TMPDIR";
        mkdir -p "$TMPDIR";
        if [ ! -d "$TMPDIR" ]; then
            echo "ERROR: Failed to create TMPDIR: $TMPDIR" >> {log} 2>&1;
        ...
        # Reheader PB CRAM to use cluster_sample SM tag so both inputs
        # share the same sample name and sentieon produces a single-sample VCF.
        PASS1_TMPDIR=$(dirname {output.vcf})
        echo "Reheadering PB CRAM to use sample name: {params.cluster_sample}" >> {log} 2>&1
        samtools view -H {input.lr_cram} | \
            sed "s/SM:[^\\t]*/SM:{params.cluster_sample}/g" > "$PASS1_TMPDIR/pb_pass1_header.txt"
        samtools reheader "$PASS1_TMPDIR/pb_pass1_header.txt" {input.lr_cram} > "$PASS1_TMPDIR/pb_pass1_reheadered.bam"
        samtools index -@ {params.use_threads} "$PASS1_TMPDIR/pb_pass1_reheadered.bam" >> {log} 2>&1

        sentieon driver -r {params.huref} -t {params.use_threads} \
            -i "$PASS1_TMPDIR/pb_pass1_reheadered.bam" -i {input.sr_bam} \
            {params.diploid_bed} \
            --algo DNAscope \
        ...
        # Cleanup reheadered BAM
        rm -f "$PASS1_TMPDIR/pb_pass1_header.txt" "$PASS1_TMPDIR/pb_pass1_reheadered.bam" "$PASS1_TMPDIR/pb_pass1_reheadered.bam.bai"
...
rule sentdhipm_hybrid_select:
    ...
    output:
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/selected.bed",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHIPM)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/log/{sample}.{alnr}.{ddup}.{dchrm}.hybrid_select.log",
    threads: config['sentdhipm']['threads']
    conda:
        "../envs/sentieon_v0.3.yaml"
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhipm.{dchrm}.hybrid_select.bench.tsv"
    resources:
        partition="i192mem,i192bigmem",
        threads=config['sentdhipm']['threads'],
        vcpu=config['sentdhipm']['threads'],
...
rule sentdhipm_mapq0_bed:
    ...
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHIPM)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/log/{sample}.{alnr}.{ddup}.{dchrm}.mapq0_bed.log",
    threads: config['sentdhipm']['threads']
    conda:
        "../envs/sentieon_v0.3.yaml"
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhipm.{dchrm}.mapq0_bed.bench.tsv"
    resources:
        partition="i192mem,i192bigmem",
        threads=config['sentdhipm']['threads'],
        vcpu=config['sentdhipm']['threads'],
...
# ---------------------------------------------------------------------------
rule sentdhipm_mapq0_slop:
    """Extend MAPQ0 regions by 1000 bp using bedtools slop"""
    input:
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/hybrid_mapq0.bed",
        ref_fai=config["supporting_files"]["files"]["huref"]["fasta"]["name"] + ".fai",
    output:
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/hybrid_mapq0.ex1000.bed",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHIPM)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/log/{sample}.{alnr}.{ddup}.{dchrm}.mapq0_slop.log",
    threads: 2
    conda:
...
# ---------------------------------------------------------------------------
rule sentdhipm_merge_beds:
    """Cat, sort, merge selected.bed + mapq0.ex1000.bed → merged_diff.bed"""
    input:
        selected=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/selected.bed",
        mapq0=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/hybrid_mapq0.ex1000.bed",
        ref_fai=config["supporting_files"]["files"]["huref"]["fasta"]["name"] + ".fai",
    output:
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/merged_diff.bed",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHIPM)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/log/{sample}.{alnr}.{ddup}.{dchrm}.merge_beds.log",
    threads: 2
    conda:
        "../envs/vanilla_v0.1.yaml"
    resources:
...
rule sentdhipm_stage1:
    ...
    output:
        ...
        hap_bam=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/stage1_hap.bam",
        hap_bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/stage1_hap.bed",
        hap_vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/stage1_hap.vcf",
        ins_fa=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/stage1_ins.fa",
        ins_bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/stage1_ins.bed",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHIPM)
    log:
    ...
    shell:
        ...
        timestamp=$(date +%Y%m%d%H%M%S);
        export TMPDIR="/dev/shm/sentdhipm_s1_${{timestamp}}_$$";
        export SENTIEON_TMPDIR="$TMPDIR";
        mkdir -p "$TMPDIR";
        if [ ! -d "$TMPDIR" ]; then
            echo "ERROR: Failed to create TMPDIR: $TMPDIR" >> {log} 2>&1;
...
# ---------------------------------------------------------------------------
rule sentdhipm_stage2:
    """Stage2: generate unmap BAM, alt BAM, and refined BED"""
    input:
        stage1_bam=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/hybrid_stage1.bam",
        hap_bam=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/stage1_hap.bam",
        hap_bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/stage1_hap.bed",
    output:
    ...
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHIPM)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/log/{sample}.{alnr}.{ddup}.{dchrm}.stage2.log",
    threads: config['sentdhipm']['threads']
    conda:
        "../envs/sentieon_v0.3.yaml"
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhipm.{dchrm}.stage2.bench.tsv"
    resources:
        partition="i192mem,i192bigmem",
        threads=config['sentdhipm']['threads'],
        vcpu=config['sentdhipm']['threads'],
...
rule sentdhipm_stage3:
    ...
    input:
        ...
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/hybrid_stage2.bed",
    output:
        bam=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/hybrid_stage3.bam",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHIPM)
    log:
...
rule sentdhipm_pass2:
    ...
    input:
        ...
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/hybrid_stage2.bed",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/hybrid_pass2.vcf.gz",
        tbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/hybrid_pass2.vcf.gz.tbi",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHIPM)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/log/{sample}.{alnr}.{ddup}.{dchrm}.pass2.log",
    threads: config['sentdhipm']['threads']
    conda:
        "../envs/sentieon_v0.3.yaml"
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhipm.{dchrm}.pass2.bench.tsv"
    resources:
        partition="i192mem,i192bigmem",
        threads=config['sentdhipm']['threads'],
        vcpu=config['sentdhipm']['threads'],
    ...
    shell:
        ...
        # Reheader PB CRAM and stage3 BAM to use cluster_sample SM tag.
        TMPDIR=$(dirname {output.vcf})

        echo "Reheadering PB CRAM to use sample name: {params.cluster_sample}" >> {log} 2>&1
        samtools view -H {input.lr_cram} | \
            sed "s/SM:[^\\t]*/SM:{params.cluster_sample}/g" > "$TMPDIR/pb_pass2_header.txt"
        samtools reheader "$TMPDIR/pb_pass2_header.txt" {input.lr_cram} > "$TMPDIR/pb_pass2_reheadered.bam"
        samtools index -@ {params.use_threads} "$TMPDIR/pb_pass2_reheadered.bam" >> {log} 2>&1
        ...
        sentieon driver -r {params.huref} -t {params.use_threads} \
            -i "$TMPDIR/pb_pass2_reheadered.bam" -i "$TMPDIR/stage3_pass2_reheadered.bam" \
            --interval {input.bed} \
            {params.diploid_bed} \
            --algo DNAscope \
            --model {params.model}/hybrid.model \
        ...
        # Cleanup temp files
        rm -f "$TMPDIR/pb_pass2_header.txt" "$TMPDIR/pb_pass2_reheadered.bam" "$TMPDIR/pb_pass2_reheadered.bam.bai"
...
# ---------------------------------------------------------------------------
rule sentdhipm_subset:
    """Subset pass-1 VCF to complement of stage2 regions"""
    input:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/initial.vcf.gz",
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/hybrid_stage2.bed",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/mix_subset.vcf.gz",
        tbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/mix_subset.vcf.gz.tbi",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHIPM)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/log/{sample}.{alnr}.{ddup}.{dchrm}.subset.log",
    threads: 4
    conda:
        "../envs/sentieon_v0.3.yaml"
    resources:
...
# ---------------------------------------------------------------------------
rule sentdhipm_concat_pass:
    """Concatenate subset + pass2 VCFs"""
    input:
        subset=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/mix_subset.vcf.gz",
        subset_tbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/mix_subset.vcf.gz.tbi",
        pass2=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/hybrid_pass2.vcf.gz",
        pass2_tbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/hybrid_pass2.vcf.gz.tbi",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/combined_tmp.vcf.gz",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHIPM)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/log/{sample}.{alnr}.{ddup}.{dchrm}.concat_pass.log",
    threads: 4
    conda:
        "../envs/vanilla_v0.1.yaml"
    resources:
...
rule sentdhipm_anno:
    ...
    input:
        ...
        hap_bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/stage1_hap.bed",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/combined_tmp_anno.vcf.gz",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHIPM)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/log/{sample}.{alnr}.{ddup}.{dchrm}.anno.log",
    threads: config['sentdhipm']['threads']
    conda:
        "../envs/sentieon_v0.3.yaml"
    resources:
...
# ---------------------------------------------------------------------------
rule sentdhipm_transfer:
    """Transfer annotations from population VCF using bcftools merge"""
    input:
        anno_vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/combined_tmp_anno.vcf.gz",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/combined_tmp_transfer.vcf.gz",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHIPM)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/log/{sample}.{alnr}.{ddup}.{dchrm}.transfer.log",
    threads: config['sentdhipm']['threads']
    conda:
        "../envs/sentieon_v0.3.yaml"
    resources:
...
# ---------------------------------------------------------------------------
rule sentdhipm_model_apply:
    """Apply ML model to called variants"""
    input:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/combined_tmp_transfer.vcf.gz",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/combined_apply.vcf.gz",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHIPM)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/log/{sample}.{alnr}.{ddup}.{dchrm}.model_apply.log",
    threads: config['sentdhipm']['threads']
    conda:
        "../envs/sentieon_v0.3.yaml"
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhipm.{dchrm}.model_apply.bench.tsv"
    resources:
        partition="i192mem,i192bigmem",
        threads=config['sentdhipm']['threads'],
        vcpu=config['sentdhipm']['threads'],
...
# ---------------------------------------------------------------------------
rule sentdhipm_final_norm:
    """Trim, normalize, and produce final output VCF"""
    input:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/tmp/combined_apply.vcf.gz",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/{sample}.{alnr}.{ddup}.sentdhipm.{dchrm}.snv.sort.vcf.gz",
        tbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/vcfs/{dchrm}/{sample}.{alnr}.{ddup}.sentdhipm.{dchrm}.snv.sort.vcf.gz.tbi",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHIPM)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/log/{sample}.{alnr}.{ddup}.{dchrm}.final_norm.log",
    threads: config['sentdhipm']['threads']
    conda:
        "../envs/sentieon_v0.3.yaml"
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhipm.{dchrm}.final_norm.bench.tsv"
    resources:
        partition="i192mem,i192bigmem",
        threads=config['sentdhipm']['threads'],
        vcpu=config['sentdhipm']['threads'],
...
localrules:
    sentdhipm_concat_fofn,


rule sentdhipm_concat_fofn:
    """Build file-of-filenames for chromosome chunks"""
    input:
        chunk_tbi=sorted(
            expand(
                MDIR
                + "{{sample}}/align/{{alnr}}/{{ddup}}/snv/sentdhipm/vcfs/{ochm}/{{sample}}.{{alnr}}.{{ddup}}.sentdhipm.{ochm}.snv.sort.vcf.gz.tbi",
                ochm=SENTDHIPM_CHRMS,
            ),
            key=lambda x: float(
    ...
    priority: 44
    output:
        fin_fofn=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/{sample}.{alnr}.{ddup}.sentdhipm.snv.concat.vcf.gz.fofn",
        tmp_fofn=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/{sample}.{alnr}.{ddup}.sentdhipm.snv.concat.vcf.gz.fofn.tmp",
    threads: 1
    resources:
        threads=1
    params:
        fn_stub="{sample}.{alnr}.{ddup}.sentdhipm."
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhipm.concat.fofn.bench.tsv"
    conda:
        "../envs/vanilla_v0.1.yaml"
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/log/{sample}.{alnr}.{ddup}.sentdhipm.concat.fofn.log",
    shell:
        """
        ...
        (workflow/scripts/sort_concat_chrm_list.py {output.tmp_fofn} {wildcards.sample}.{wildcards.alnr}.{wildcards.ddup}.sentdhipm. {output.fin_fofn}) >> {log} 2>&1;
        """


rule sentdhipm_concat_index_chunks:
    """Concatenate chromosome chunks and index final VCF"""
    input:
        fofn=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/{sample}.{alnr}.{ddup}.sentdhipm.snv.concat.vcf.gz.fofn",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHIPM)
    output:
        vcfgz=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/{sample}.{alnr}.{ddup}.sentdhipm.snv.sort.vcf.gz",
        vcfgztemp=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/{sample}.{alnr}.{ddup}.sentdhipm.snv.sort.temp.vcf.gz",
        vcfgztbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/{sample}.{alnr}.{ddup}.sentdhipm.snv.sort.vcf.gz.tbi",
    threads: config['sentdhipm']['threads']
    resources:
        vcpu=config['sentdhipm']['threads'],
        threads=config['sentdhipm']['threads'],
    ...
    params:
        ...
        cluster_sample=ret_sample,
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhipm.merge.bench.tsv"
    conda:
        "../envs/vanilla_v0.1.yaml"
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/log/{sample}.{alnr}.{ddup}.sentdhipm.snv.merge.sort.gathered.log",
    shell:
        """
        touch {log};
        mkdir -p $(dirname {log});
...
localrules:
    clear_combined_sentdhipm_vcf,


rule clear_combined_sentdhipm_vcf:  # TARGET: clear combined sentdhipm vcf so chunks can be re-evaluated if needed.
    input:
        expand(
            MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/{sample}.{alnr}.{ddup}.sentdhipm.snv.sort.vcf.gz",
            sample=SSAMPS,
            alnr=ALIGNERS_DHIPM,
            ddup=DDUP,
        ),
...
localrules:
    produce_sentdhipm_vcf,


rule produce_sentdhipm_vcf:  # TARGET: sentieon dnascope hybrid modular vcf
    input:
        expand(
            MDIR
            + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/{sample}.{alnr}.{ddup}.sentdhipm.snv.sort.vcf.gz.tbi",
            sample=SSAMPS,
            alnr=ALIGNERS_DHIPM,
            ddup=DDUP,
        ),
    output:
        "gatheredall.sentdhipm",
    priority: 48
    threads: 1
    log:
        "gatheredall.sentdhipm.log",
    shell:
        """( touch {output} ;
...
localrules:
    prep_sentdhipm_chunkdirs,


rule prep_sentdhipm_chunkdirs:
    ...
    output:
        expand(
            ...
            dchrm=SENTDHIPM_CHRMS
        ),
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHIPM)
    threads: 1
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhipm/logs/{sample}.{alnr}.{ddup}.chunkdirs.log",
    shell:
        """
        ( echo {output}  ;
