...
def get_varscan2_chrm_day(wildcards):
    pchr=""  # prefix handled already
    ret_str = ""
    sl = wildcards.varscan2chrm.replace('chr', '').split("-")
    sl2 = wildcards.varscan2chrm.replace('chr', '').split("~")

    if len(sl2) == 2:
        ret_str = pchr + wildcards.varscan2chrm
    elif len(sl) == 1:
        ret_str = pchr + sl[0]
    elif len(sl) == 2:
        start = int(sl[0])
        ...
        raise Exception(
            "varscan2 chunks can only be one contiguous range per chunk : ie: 1-4 with the non numerical chrms assigned 23=X, 24=Y,25=MT"
