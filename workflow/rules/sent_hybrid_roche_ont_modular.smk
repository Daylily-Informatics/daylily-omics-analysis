...
Caller code: sentdhrom (Sentieon DNAscope Hybrid Roche+ONT Modular)
...
- Uses ALIGNERS_DHROM = ["roche"] for wildcard constraints
...
rule sentdhrom_pass1:
    ...
    input:
        ...
        d=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/{sample}.ready",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/initial.vcf.gz",
        tbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/initial.vcf.gz.tbi",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHROM)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/log/{sample}.{alnr}.{ddup}.{dchrm}.pass1.log",
    threads: config['sentdhrom']['threads']
    conda:
        "../envs/sentieon_v0.3.yaml"
    priority: 45
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhrom.{dchrm}.pass1.bench.tsv"
    resources:
    ...
    shell:
        ...
        export TMPDIR="/dev/shm/sentdhrom_p1_${{timestamp}}_$$";
        ...
        # Reheader input files to use consistent sample name (cluster_sample)
        # This ensures DNAscope produces a single-sample VCF
        echo "Reheadering input files to use sample name: {params.cluster_sample}" >> {log} 2>&1

        # Create temporary BAMs with corrected sample names
        samtools view -H {input.roche_bam} | \
            sed "s/SM:[^\\t]*/SM:{params.cluster_sample}/g" > "$TMPDIR/roche_header.txt"
        samtools reheader "$TMPDIR/roche_header.txt" {input.roche_bam} > "$TMPDIR/roche_reheadered.bam"
        samtools index -@ {params.use_threads} "$TMPDIR/roche_reheadered.bam" >> {log} 2>&1
        ...
        sentieon driver -r {params.huref} -t {params.use_threads} \
            -i "$TMPDIR/ont_reheadered.bam" -i "$TMPDIR/roche_reheadered.bam" \
            {params.diploid_bed} \
            --algo DNAscope \
...
rule sentdhrom_hybrid_select:
    ...
    output:
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/selected.bed",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHROM)
    log:
...
# ---------------------------------------------------------------------------
rule sentdhrom_mapq0_slop:
    """Extend MAPQ0 regions by 1000 bp using bedtools slop"""
    input:
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/hybrid_mapq0.bed",
        ref_fai=config["supporting_files"]["files"]["huref"]["fasta"]["name"] + ".fai",
    output:
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/hybrid_mapq0.ex1000.bed",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHROM)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/log/{sample}.{alnr}.{ddup}.{dchrm}.mapq0_slop.log",
    threads: 2
    conda:
...
# ---------------------------------------------------------------------------
rule sentdhrom_merge_beds:
    """Cat, sort, merge selected.bed + mapq0.ex1000.bed → merged_diff.bed"""
    input:
        selected=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/selected.bed",
        mapq0=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/hybrid_mapq0.ex1000.bed",
        ref_fai=config["supporting_files"]["files"]["huref"]["fasta"]["name"] + ".fai",
    output:
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/merged_diff.bed",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHROM)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/log/{sample}.{alnr}.{ddup}.{dchrm}.merge_beds.log",
    threads: 2
    conda:
        "../envs/vanilla_v0.1.yaml"
    resources:
...
# ---------------------------------------------------------------------------
rule sentdhrom_stage1:
    """Stage1: insertion detection + haplotype assembly piped through bwa"""
    input:
        ont_cram=MDIR + "{sample}/align/ont/{sample}.cram",
        diff_bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/merged_diff.bed",
    output:
        bam=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/hybrid_stage1.bam",
        hap_bam=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/stage1_hap.bam",
        hap_bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/stage1_hap.bed",
        hap_vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/stage1_hap.vcf",
        ins_fa=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/stage1_ins.fa",
        ins_bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/stage1_ins.bed",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHROM)
    log:
    ...
    shell:
        ...
        timestamp=$(date +%Y%m%d%H%M%S);
        export TMPDIR="/dev/shm/sentdhrom_s1_${{timestamp}}_$$";
        export SENTIEON_TMPDIR="$TMPDIR";
        mkdir -p "$TMPDIR";
        if [ ! -d "$TMPDIR" ]; then
            echo "ERROR: Failed to create TMPDIR: $TMPDIR" >> {log} 2>&1;
...
# ---------------------------------------------------------------------------
rule sentdhrom_stage2:
    """Stage2: generate unmap BAM, alt BAM, and refined BED"""
    input:
        stage1_bam=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/hybrid_stage1.bam",
        hap_bam=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/stage1_hap.bam",
        hap_bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/stage1_hap.bed",
    output:
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/hybrid_stage2.bed",
...
rule sentdhrom_stage3:
    ...
    input:
        ...
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/hybrid_stage2.bed",
    output:
        bam=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/hybrid_stage3.bam",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHROM)
    log:
...
# ---------------------------------------------------------------------------
rule sentdhrom_pass2:
    """Second-pass variant calling on stage3 BAM + ONT reads"""
    input:
        ont_cram=MDIR + "{sample}/align/ont/{sample}.cram",
        stage3_bam=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/hybrid_stage3.bam",
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/hybrid_stage2.bed",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/hybrid_pass2.vcf.gz",
        tbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/hybrid_pass2.vcf.gz.tbi",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHROM)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/log/{sample}.{alnr}.{ddup}.{dchrm}.pass2.log",
    threads: config['sentdhrom']['threads']
    conda:
        "../envs/sentieon_v0.3.yaml"
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhrom.{dchrm}.pass2.bench.tsv"
    resources:
        partition="i192mem,i192bigmem",
        threads=192,
        vcpu=192,
...
# ---------------------------------------------------------------------------
rule sentdhrom_subset:
    """Subset pass-1 VCF to complement of stage2 regions"""
    input:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/initial.vcf.gz",
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/hybrid_stage2.bed",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/mix_subset.vcf.gz",
        tbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/mix_subset.vcf.gz.tbi",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHROM)
    log:
...
# ---------------------------------------------------------------------------
rule sentdhrom_concat_pass:
    """Concatenate subset + pass2 VCFs"""
    input:
        subset=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/mix_subset.vcf.gz",
        subset_tbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/mix_subset.vcf.gz.tbi",
        pass2=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/hybrid_pass2.vcf.gz",
        pass2_tbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/hybrid_pass2.vcf.gz.tbi",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/combined_tmp.vcf.gz",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHROM)
    log:
...
rule sentdhrom_anno:
    ...
    input:
        ...
        hap_bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/stage1_hap.bed",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/combined_tmp_anno.vcf.gz",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHROM)
    log:
...
# ---------------------------------------------------------------------------
rule sentdhrom_transfer:
    """Transfer annotations from population VCF using bcftools merge"""
    input:
        anno_vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/combined_tmp_anno.vcf.gz",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/combined_tmp_transfer.vcf.gz",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHROM)
    log:
...
# ---------------------------------------------------------------------------
rule sentdhrom_model_apply:
    """Apply ML model to called variants"""
    input:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/combined_tmp_transfer.vcf.gz",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/combined_apply.vcf.gz",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHROM)
    log:
...
# ---------------------------------------------------------------------------
rule sentdhrom_final_norm:
    """Trim, normalize, and produce final output VCF"""
    input:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/tmp/combined_apply.vcf.gz",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/{sample}.{alnr}.{ddup}.sentdhrom.{dchrm}.snv.sort.vcf.gz",
        tbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/vcfs/{dchrm}/{sample}.{alnr}.{ddup}.sentdhrom.{dchrm}.snv.sort.vcf.gz.tbi",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHROM)
    log:
...
localrules:
    sentdhrom_concat_fofn,


rule sentdhrom_concat_fofn:
    """Build file-of-filenames for chromosome chunks"""
    input:
        chunk_tbi=sorted(
            expand(
                MDIR
                + "{{sample}}/align/{{alnr}}/{{ddup}}/snv/sentdhrom/vcfs/{ochm}/{{sample}}.{{alnr}}.{{ddup}}.sentdhrom.{ochm}.snv.sort.vcf.gz.tbi",
                ochm=SENTDHROM_CHRMS,
            ),
            key=lambda x: float(
    ...
    priority: 44
    output:
        fin_fofn=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/{sample}.{alnr}.{ddup}.sentdhrom.snv.concat.vcf.gz.fofn",
        tmp_fofn=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/{sample}.{alnr}.{ddup}.sentdhrom.snv.concat.vcf.gz.fofn.tmp",
    threads: 1
    resources:
        threads=1
    params:
        fn_stub="{sample}.{alnr}.{ddup}.sentdhrom."
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhrom.concat.fofn.bench.tsv"
    conda:
        "../envs/vanilla_v0.1.yaml"
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/log/{sample}.{alnr}.{ddup}.sentdhrom.concat.fofn.log",
    shell:
        """
        ...
        (workflow/scripts/sort_concat_chrm_list.py {output.tmp_fofn} {wildcards.sample}.{wildcards.alnr}.{wildcards.ddup}.sentdhrom. {output.fin_fofn}) >> {log} 2>&1;
        """


rule sentdhrom_concat_index_chunks:
    """Concatenate chromosome chunks and index final VCF"""
    input:
        fofn=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/{sample}.{alnr}.{ddup}.sentdhrom.snv.concat.vcf.gz.fofn",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHROM)
    output:
    ...
    params:
        ...
        cluster_sample=ret_sample,
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhrom.merge.bench.tsv"
    conda:
        "../envs/vanilla_v0.1.yaml"
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/log/{sample}.{alnr}.{ddup}.sentdhrom.snv.merge.sort.gathered.log",
    shell:
        """
        touch {log};
        mkdir -p $(dirname {log});
...
localrules:
    clear_combined_sentdhrom_vcf,


rule clear_combined_sentdhrom_vcf:  # TARGET: clear combined sentdhrom vcf so chunks can be re-evaluated if needed.
    input:
        expand(
            MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/{sample}.{alnr}.{ddup}.sentdhrom.snv.sort.vcf.gz",
            sample=SSAMPS,
            alnr=ALIGNERS_DHROM,
            ddup=DDUP,
        ),
...
localrules:
    produce_sentdhrom_vcf,


rule produce_sentdhrom_vcf:  # TARGET: sentieon dnascope hybrid roche+ont modular vcf
    input:
        expand(
            MDIR
            + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/{sample}.{alnr}.{ddup}.sentdhrom.snv.sort.vcf.gz.tbi",
            sample=SSAMPS,
            alnr=ALIGNERS_DHROM,
            ddup=DDUP,
        ),
    output:
        "gatheredall.sentdhrom",
    priority: 48
    threads: 1
    log:
        "gatheredall.sentdhrom.log",
    shell:
        """( touch {output} ;
...
localrules:
    prep_sentdhrom_chunkdirs,


rule prep_sentdhrom_chunkdirs:
    ...
    output:
        expand(
            MDIR + "{{sample}}/align/{{alnr}}/{{ddup}}/snv/sentdhrom/vcfs/{dchrm}/{{sample}}.ready",
            dchrm=SENTDHROM_CHRMS
        ),
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHROM)
    threads: 1
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhrom/logs/{sample}.{alnr}.{ddup}.chunkdirs.log",
    shell:
        """
        ( echo {output}  ;
