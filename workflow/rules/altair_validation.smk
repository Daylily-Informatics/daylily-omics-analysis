...
if "altair_validation" not in config or not isinstance(config["altair_validation"], dict):
    raise WorkflowError("produce_altair_validation_artifacts requires altair_validation config.")

ALTAIR_VALIDATION_CONFIG = config["altair_validation"]
...
def _altair_required_config_value(key):
...
ALTAIR_VALIDATION_OUTPUT_DIR = _altair_required_config_value("output_dir").rstrip("/")


def _altair_required_config_path(key):
    return _altair_required_config_value(key)
...
def _altair_coverage_param(key):
    coverage = ALTAIR_VALIDATION_CONFIG.get("coverage", {})
    if not isinstance(coverage, dict):
        raise WorkflowError("produce_altair_validation_artifacts requires altair_validation.coverage.")
    value = str(coverage.get(key, "")).strip()
    if value == "":
        raise WorkflowError(
            f"produce_altair_validation_artifacts requires altair_validation.coverage.{key}."
        )
    return value
...
rule altair_rr_mosdepth:
    ...
    params:
        ...
        mapq=lambda wildcards: _altair_coverage_param("min_mapq"),
        depth_bins=lambda wildcards: _altair_coverage_param("depth_bins"),
