...
rule clear_combined_clair3_vcf:  # TARGET : Clear combined Clair3 VCFs for re-evaluation
    ...
    benchmark:
        "logs/benchmarks/clear_combined_clair3_vcf.bench.tsv"
    priority: 42
    shell:
...
rule produce_clair3_vcf:  # DEPRECATED TARGET: use produce_clair3_snv_vcf
    ...
    benchmark:
        "logs/benchmarks/produce_clair3_vcf.bench.tsv"
    conda:
        "../envs/vanilla_v0.1.yaml"
    params:
        cluster_sample=ret_sample,
...
rule prep_clair3_chunkdirs:
    input:
        ...
        ba=MDIR + "{sample}/align/{alnr}/{ddup}/{sample}.{alnr}.{ddup}.cram.crai",
    output:
        expand(
            MDIR + "{{sample}}/align/{{alnr}}/{{ddup}}/snv/clair3/vcfs/{clairchrm}/{{sample}}.ready",
