...
Caller code: sentdhupmr (Sentieon DNAscope Hybrid Ultima+PacBio Modular)
...
- Uses ALIGNERS_DHUPMR = ["ug"] for wildcard constraints
...
rule sentdhupmr_pass1:
    ...
    input:
        ...
        d=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/{sample}.ready",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/initial.vcf.gz",
        tbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/initial.vcf.gz.tbi",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHUPMR)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/log/{sample}.{alnr}.{ddup}.{dchrm}.pass1.log",
    threads: config['sentdhupmr']['threads']
    conda:
        "../envs/sentieon_v0.3.yaml"
    priority: 45
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhupmr.{dchrm}.pass1.bench.tsv"
    resources:
    ...
    shell:
        ...
        export TMPDIR="/scratch/sentdhupmr_p1_${{timestamp}}_$$";
...
rule sentdhupmr_hybrid_select:
    ...
    output:
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/selected.bed",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHUPMR)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/log/{sample}.{alnr}.{ddup}.{dchrm}.hybrid_select.log",
    threads: config['sentdhupmr']['threads_light']
    conda:
        "../envs/sentieon_v0.3.yaml"
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhupmr.{dchrm}.hybrid_select.bench.tsv"
    resources:
        partition="i192hugenvme,i192nvme,i384nvme",
        threads=config['sentdhupmr']['threads_light'],
    ...
    shell:
        ...
        : "${{CONDA_PREFIX:?CONDA_PREFIX is required for sentdhupmr_hybrid_select}}"
        {{
...
# ---------------------------------------------------------------------------
rule sentdhupmr_mapq0_bed:
    """Detect MAPQ0 regions with HybridStage2 region model"""
    input:
        ug_cram=MDIR + "{sample}/align/{alnr}/{sample}.cram",
        pb_cram=MDIR + "{sample}/align/sentmm2/{sample}.sentmm2.cram",
    output:
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/hybrid_mapq0.bed",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHUPMR)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/log/{sample}.{alnr}.{ddup}.{dchrm}.mapq0_bed.log",
    threads: config['sentdhupmr']['threads_medium']
    conda:
        "../envs/sentieon_v0.3.yaml"
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhupmr.{dchrm}.mapq0_bed.bench.tsv"
    resources:
        partition="i192hugenvme,i192nvme,i384nvme",
        threads=config['sentdhupmr']['threads_medium'],
        vcpu=config['sentdhupmr']['threads_medium'],
        mem_mb=config['sentdhupmr']['mem_mb_medium'],
    params:
        huref=config["supporting_files"]["files"]["huref"]["fasta"]["name"],
        model=config["sentdhupmr"]["dna_scope_snv_model"],
        diploid_bed=get_diploid_bed_path,
    ...
    shell:
        ...
        timestamp=$(date +%Y%m%d%H%M%S);
        export TMPDIR="/scratch/sentdhupmr_mq_${{timestamp}}_$$";
        export SENTIEON_TMPDIR="$TMPDIR";
        mkdir -p "$TMPDIR";
        trap 'rm -rf "$TMPDIR" 2>/dev/null || true' EXIT;
...
# ---------------------------------------------------------------------------
rule sentdhupmr_mapq0_slop:
    """Extend MAPQ0 regions by 1000 bp using bedtools slop"""
    input:
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/hybrid_mapq0.bed",
        ref_fai=config["supporting_files"]["files"]["huref"]["fasta"]["name"] + ".fai",
    output:
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/hybrid_mapq0.ex1000.bed",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHUPMR)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/log/{sample}.{alnr}.{ddup}.{dchrm}.mapq0_slop.log",
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.{dchrm}.sentdhupmr_mapq0_slop.bench.tsv"
    threads: 2
    conda:
...
# ---------------------------------------------------------------------------
rule sentdhupmr_merge_beds:
    """Cat, sort, merge selected.bed + mapq0.ex1000.bed → merged_diff.bed"""
    input:
        selected=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/selected.bed",
        mapq0=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/hybrid_mapq0.ex1000.bed",
        ref_fai=config["supporting_files"]["files"]["huref"]["fasta"]["name"] + ".fai",
    output:
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/merged_diff.bed",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHUPMR)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/log/{sample}.{alnr}.{ddup}.{dchrm}.merge_beds.log",
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.{dchrm}.sentdhupmr_merge_beds.bench.tsv"
    threads: 2
    conda:
        "../envs/vanilla_v0.1.yaml"
    resources:
...
# ---------------------------------------------------------------------------
rule sentdhupmr_stage1:
    """Stage1: insertion detection + haplotype assembly piped through bwa"""
    input:
        pb_cram=MDIR + "{sample}/align/sentmm2/{sample}.sentmm2.cram",
        diff_bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/merged_diff.bed",
    output:
        bam=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/hybrid_stage1.bam",
        hap_bam=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/stage1_hap.bam",
        hap_bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/stage1_hap.bed",
        hap_vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/stage1_hap.vcf",
        ins_fa=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/stage1_ins.fa",
        ins_bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/stage1_ins.bed",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHUPMR)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/log/{sample}.{alnr}.{ddup}.{dchrm}.stage1.log",
    threads: config['sentdhupmr']['threads']  # Full node: stage1 runs 4 concurrent processes (HAP+INS+bwa+sort)
    conda:
        "../envs/sentieon_v0.3.yaml"
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhupmr.{dchrm}.stage1.bench.tsv"
    resources:
        partition="i192hugenvme,i192nvme,i384nvme",
        threads=config['sentdhupmr']['threads'],
        vcpu=config['sentdhupmr']['threads'],
    ...
    shell:
        ...
        timestamp=$(date +%Y%m%d%H%M%S);
        export TMPDIR="/scratch/sentdhupmr_s1_${{timestamp}}_$$";
        export SENTIEON_TMPDIR="$TMPDIR";
        mkdir -p "$TMPDIR";
        if [ ! -d "$TMPDIR" ]; then
            echo "ERROR: Failed to create TMPDIR: $TMPDIR" >> {log} 2>&1;
...
# ---------------------------------------------------------------------------
rule sentdhupmr_stage2:
    """Stage2: generate unmap BAM, alt BAM, and refined BED"""
    input:
        stage1_bam=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/hybrid_stage1.bam",
        hap_bam=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/stage1_hap.bam",
        hap_bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/stage1_hap.bed",
    output:
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/hybrid_stage2.bed",
        unmap_bam=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/hybrid_stage2_unmap.bam",
        alt_bam=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/hybrid_stage2_alt.bam",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHUPMR)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/log/{sample}.{alnr}.{ddup}.{dchrm}.stage2.log",
    threads: config['sentdhupmr']['threads_medium']
    conda:
        "../envs/sentieon_v0.3.yaml"
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhupmr.{dchrm}.stage2.bench.tsv"
    resources:
        partition="i192hugenvme,i192nvme,i384nvme",
        threads=config['sentdhupmr']['threads_medium'],
        vcpu=config['sentdhupmr']['threads_medium'],
    ...
    shell:
        ...
        timestamp=$(date +%Y%m%d%H%M%S);
        export TMPDIR="/scratch/sentdhupmr_s2_${{timestamp}}_$$";
        export SENTIEON_TMPDIR="$TMPDIR";
        mkdir -p "$TMPDIR";
        trap 'rm -rf "$TMPDIR" 2>/dev/null || true' EXIT;
...
rule sentdhupmr_stage3:
    ...
    input:
        ...
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/hybrid_stage2.bed",
    output:
        bam=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/hybrid_stage3.bam",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHUPMR)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/log/{sample}.{alnr}.{ddup}.{dchrm}.stage3.log",
    threads: config['sentdhupmr']['threads']  # Full node: stage3 pipes driver → util sort (2 concurrent processes)
    conda:
        "../envs/sentieon_v0.3.yaml"
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhupmr.{dchrm}.stage3.bench.tsv"
    resources:
        partition="i192hugenvme,i192nvme,i384nvme",
        threads=config['sentdhupmr']['threads'],
        vcpu=config['sentdhupmr']['threads'],
    ...
    shell:
        ...
        timestamp=$(date +%Y%m%d%H%M%S);
        export TMPDIR="/scratch/sentdhupmr_s3_${{timestamp}}_$$";
        export SENTIEON_TMPDIR="$TMPDIR";
        mkdir -p "$TMPDIR";
        trap 'rm -rf "$TMPDIR" 2>/dev/null || true' EXIT;
...
# ---------------------------------------------------------------------------
rule sentdhupmr_pass2:
    """Second-pass variant calling on stage3 BAM + ONT reads"""
    input:
        pb_cram=MDIR + "{sample}/align/sentmm2/{sample}.sentmm2.cram",
        stage3_bam=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/hybrid_stage3.bam",
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/hybrid_stage2.bed",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/hybrid_pass2.vcf.gz",
        tbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/hybrid_pass2.vcf.gz.tbi",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHUPMR)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/log/{sample}.{alnr}.{ddup}.{dchrm}.pass2.log",
    threads: config['sentdhupmr']['threads']
    conda:
        "../envs/sentieon_v0.3.yaml"
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhupmr.{dchrm}.pass2.bench.tsv"
    resources:
        partition="i192hugenvme,i192nvme,i384nvme",
        threads=config['sentdhupmr']['threads'],
        vcpu=config['sentdhupmr']['threads'],
    ...
    shell:
        ...
        timestamp=$(date +%Y%m%d%H%M%S);
        export TMPDIR="/scratch/sentdhupmr_p2_${{timestamp}}_$$";
        export SENTIEON_TMPDIR="$TMPDIR";
        mkdir -p "$TMPDIR";
        trap 'rm -rf "$TMPDIR" 2>/dev/null || true' EXIT;
...
# ---------------------------------------------------------------------------
rule sentdhupmr_subset:
    """Subset pass-1 VCF to complement of stage2 regions"""
    input:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/initial.vcf.gz",
        bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/hybrid_stage2.bed",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/mix_subset.vcf.gz",
        tbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/mix_subset.vcf.gz.tbi",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHUPMR)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/log/{sample}.{alnr}.{ddup}.{dchrm}.subset.log",
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.{dchrm}.sentdhupmr_subset.bench.tsv"
    threads: config['sentdhupmr']['threads_light']
    conda:
        "../envs/sentieon_v0.3.yaml"
    resources:
...
# ---------------------------------------------------------------------------
rule sentdhupmr_concat_pass:
    """Concatenate subset + pass2 VCFs"""
    input:
        subset=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/mix_subset.vcf.gz",
        subset_tbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/mix_subset.vcf.gz.tbi",
        pass2=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/hybrid_pass2.vcf.gz",
        pass2_tbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/hybrid_pass2.vcf.gz.tbi",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/combined_tmp.vcf.gz",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHUPMR)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/log/{sample}.{alnr}.{ddup}.{dchrm}.concat_pass.log",
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.{dchrm}.sentdhupmr_concat_pass.bench.tsv"
    threads: config['sentdhupmr']['threads_light']
    conda:
        "../envs/vanilla_v0.1.yaml"
    resources:
...
# ---------------------------------------------------------------------------
rule sentdhupmr_anno:
    """Annotate VCF with hybrid-specific annotations"""
    input:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/combined_tmp.vcf.gz",
        hap_bed=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/stage1_hap.bed",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/combined_tmp_anno.vcf.gz",
        tbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/combined_tmp_anno.vcf.gz.tbi",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHUPMR)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/log/{sample}.{alnr}.{ddup}.{dchrm}.anno.log",
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.{dchrm}.sentdhupmr_anno.bench.tsv"
    threads: config['sentdhupmr']['threads_light']
    conda:
        "../envs/sentieon_v0.3.yaml"
    resources:
    ...
    shell:
        ...
        : "${{CONDA_PREFIX:?CONDA_PREFIX is required for sentdhupmr_anno}}"
...
rule sentdhupmr_transfer:
    ...
    shell:
        ...
        : "${{CONDA_PREFIX:?CONDA_PREFIX is required for sentdhupmr_transfer_anno_shards}}"
...
# ---------------------------------------------------------------------------
rule sentdhupmr_model_apply:
    """Apply ML model to called variants"""
    input:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/combined_tmp_transfer.vcf.gz",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/combined_apply.vcf.gz",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHUPMR)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/log/{sample}.{alnr}.{ddup}.{dchrm}.model_apply.log",
    threads: config['sentdhupmr']['threads_medium']
    conda:
        "../envs/sentieon_v0.3.yaml"
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhupmr.{dchrm}.model_apply.bench.tsv"
    resources:
        partition="i192hugenvme,i192nvme,i384nvme",
        threads=config['sentdhupmr']['threads_medium'],
        vcpu=config['sentdhupmr']['threads_medium'],
    ...
    shell:
        ...
        timestamp=$(date +%Y%m%d%H%M%S);
        export TMPDIR="/scratch/sentdhupmr_ma_${{timestamp}}_$$";
        export SENTIEON_TMPDIR="$TMPDIR";
        mkdir -p "$TMPDIR";
        trap 'rm -rf "$TMPDIR" 2>/dev/null || true' EXIT;
...
# ---------------------------------------------------------------------------
rule sentdhupmr_final_norm:
    """Trim, normalize, and produce final output VCF"""
    input:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/tmp/combined_apply.vcf.gz",
    output:
        vcf=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/{sample}.{alnr}.{ddup}.sentdhupmr.{dchrm}.snv.sort.vcf.gz",
        tbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/vcfs/{dchrm}/{sample}.{alnr}.{ddup}.sentdhupmr.{dchrm}.snv.sort.vcf.gz.tbi",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHUPMR)
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/log/{sample}.{alnr}.{ddup}.{dchrm}.final_norm.log",
    threads: config['sentdhupmr']['threads_light']
    conda:
        "../envs/sentieon_v0.3.yaml"
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhupmr.{dchrm}.final_norm.bench.tsv"
    resources:
        partition="i192hugenvme,i192nvme,i384nvme",
        threads=config['sentdhupmr']['threads_light'],
        vcpu=config['sentdhupmr']['threads_light'],
...
localrules:
    sentdhupmr_concat_fofn,


rule sentdhupmr_concat_fofn:
    """Build file-of-filenames for chromosome chunks"""
    input:
        chunk_tbi=sorted(
            expand(
                MDIR
                + "{{sample}}/align/{{alnr}}/{{ddup}}/snv/sentdhupmr/vcfs/{ochm}/{{sample}}.{{alnr}}.{{ddup}}.sentdhupmr.{ochm}.snv.sort.vcf.gz.tbi",
                ochm=SENTDHUPMR_CHRMS,
            ),
            key=lambda x: float(
    ...
    priority: 44
    output:
        fin_fofn=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/{sample}.{alnr}.{ddup}.sentdhupmr.snv.concat.vcf.gz.fofn",
        tmp_fofn=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/{sample}.{alnr}.{ddup}.sentdhupmr.snv.concat.vcf.gz.fofn.tmp",
    threads: 1
    resources:
        threads=1
    params:
        fn_stub="{sample}.{alnr}.{ddup}.sentdhupmr."
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhupmr.concat.fofn.bench.tsv"
    conda:
        "../envs/vanilla_v0.1.yaml"
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/log/{sample}.{alnr}.{ddup}.sentdhupmr.concat.fofn.log",
    shell:
        """
        ...
        (workflow/scripts/sort_concat_chrm_list.py {output.tmp_fofn} {wildcards.sample}.{wildcards.alnr}.{wildcards.ddup}.sentdhupmr. {output.fin_fofn}) >> {log} 2>&1;
        """


rule sentdhupmr_concat_index_chunks:
    """Concatenate chromosome chunks and index final VCF"""
    input:
        fofn=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/{sample}.{alnr}.{ddup}.sentdhupmr.snv.concat.vcf.gz.fofn",
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHUPMR)
    output:
        vcfgz=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/{sample}.{alnr}.{ddup}.sentdhupmr.snv.sort.vcf.gz",
        vcfgztemp=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/{sample}.{alnr}.{ddup}.sentdhupmr.snv.sort.temp.vcf.gz",
        vcfgztbi=MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/{sample}.{alnr}.{ddup}.sentdhupmr.snv.sort.vcf.gz.tbi",
    threads: config['sentdhupmr']['threads_light']
    resources:
        vcpu=config['sentdhupmr']['threads_light'],
        threads=config['sentdhupmr']['threads_light'],
    ...
    params:
        ...
        cluster_sample=ret_sample,
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.sentdhupmr.merge.bench.tsv"
    conda:
        "../envs/vanilla_v0.1.yaml"
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/log/{sample}.{alnr}.{ddup}.sentdhupmr.snv.merge.sort.gathered.log",
    shell:
        """
        touch {log};
        mkdir -p $(dirname {log});
...
localrules:
    clear_combined_sentdhupmr_vcf,


rule clear_combined_sentdhupmr_vcf:  # TARGET: clear combined sentdhupmr vcf so chunks can be re-evaluated if needed.
    input:
        expand(
            MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/{sample}.{alnr}.{ddup}.sentdhupmr.snv.sort.vcf.gz",
            sample=SSAMPS,
    ...
    benchmark:
        "logs/benchmarks/clear_combined_sentdhupmr_vcf.bench.tsv"
    threads: 2
    priority: 42
    shell:
        """
...
localrules:
    produce_sentdhupmr_vcf,


rule produce_sentdhupmr_vcf:  # TARGET: sentieon dnascope hybrid ultima+ont modular vcf
    input:
        expand(
            MDIR
            + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/{sample}.{alnr}.{ddup}.sentdhupmr.snv.sort.vcf.gz.tbi",
            sample=SSAMPS,
            alnr=ALIGNERS_DHUPMR,
            ddup=DDUP,
        ),
    output:
        "gatheredall.sentdhupmr",
    priority: 48
    threads: 1
    log:
        "gatheredall.sentdhupmr.log",
    benchmark:
        "logs/benchmarks/produce_sentdhupmr_vcf.bench.tsv"
    shell:
        """( touch {output} ;
...
rule sentdhupmr_call_svs:
    ...
    shell:
        ...
        timestamp=$(date +%Y%m%d%H%M%S);
        export TMPDIR="/scratch/sentdhupmr_sv_${{timestamp}}_$$";
        export SENTIEON_TMPDIR="$TMPDIR";
        mkdir -p "$TMPDIR";
        trap 'rm -rf "$TMPDIR" 2>/dev/null || true' EXIT;
...
rule produce_sentdhupmr_sv:  # TARGET: sentieon longreadsv hybrid ultima+pb modular sv vcf
    ...
    benchmark:
        "logs/benchmarks/produce_sentdhupmr_sv.bench.tsv"
    shell:
        """( touch {output} ;
        ls {output} ) >> {log} 2>&1;
        """
...
localrules:
    prep_sentdhupmr_chunkdirs,


rule prep_sentdhupmr_chunkdirs:
    ...
    output:
        expand(
            MDIR + "{{sample}}/align/{{alnr}}/{{ddup}}/snv/sentdhupmr/vcfs/{dchrm}/{{sample}}.ready",
            dchrm=SENTDHUPMR_CHRMS
        ),
    wildcard_constraints:
        alnr="|".join(ALIGNERS_DHUPMR)
    threads: 1
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/sentdhupmr/logs/{sample}.{alnr}.{ddup}.chunkdirs.log",
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.prep_sentdhupmr_chunkdirs.bench.tsv"
    shell:
        """
        ( echo {output}  ;
