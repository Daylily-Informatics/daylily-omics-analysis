...
rule pre_prep_ultima_cram:
    ...
    resources:
        ...
        constraint=config['prep_input_sample_files']['constraint'],
        distribution=config['prep_input_sample_files']['distribution'],
        exclude=config['prep_input_sample_files']['exclude'],
        include=config['prep_input_sample_files']['include'],
        exclusive=config['prep_input_sample_files']['exclusive'],
    params:
        c=config["prep_input_sample_files"]["source_read_method"],
        downsample=get_ultima_downsample,
        huref="/fsx/references/genomic_data/organism_references/H_sapiens/hg38_broad/Homo_sapiens_assembly38.fasta",  # Hardcoded for Ultima CRAM compatibility
        cluster_sample=ret_sample,
...
rule pre_prep_roche_bam:
    ...
    resources:
        ...
        constraint=config['roche_downsample_bam']['constraint'],
        distribution=config['roche_downsample_bam']['distribution'],
        exclude=config['roche_downsample_bam']['exclude'],
        include=config['roche_downsample_bam']['include'],
        exclusive=config['roche_downsample_bam']['exclusive'],
    params:
