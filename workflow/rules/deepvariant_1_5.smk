...
rule deepvariant_15:
    input:
        ...
        d=MDIR + "{sample}/align/{alnr}/{ddup}/snv/deep15/vcfs/{dvchrm}/{sample}.ready",
    output:
        vcf=temp(MDIR
        + "{sample}/align/{alnr}/{ddup}/snv/deep15/vcfs/{dvchrm}/{sample}.{alnr}.deep15.{dvchrm}.snv.vcf"),
        #gvcf=temp(MDIR + "{sample}/align/{alnr}/{ddup}/snv/deep15/vcfs/{dvchrm}/{sample}.{alnr}.deep15.{dvchrm}.snv.g.vcf"),
    log:
        MDIR
        + "{sample}/align/{alnr}/{ddup}/snv/deep15/log/{sample}.{alnr}.deep15.{dvchrm}.snv.log",
    threads: config['deepvariant']['threads']
    container:
        config['deepvariant']['deep15_container']
    priority: 45
    resources:
...
rule deep15_sort_index_chunk_vcf:
    input:
        vcf=MDIR
        + "{sample}/align/{alnr}/{ddup}/snv/deep15/vcfs/{dvchrm}/{sample}.{alnr}.deep15.{dvchrm}.snv.vcf",
    priority: 46
    output:
        vcfsort=MDIR
        + "{sample}/align/{alnr}/{ddup}/snv/deep15/vcfs/{dvchrm}/{sample}.{alnr}.deep15.{dvchrm}.snv.sort.vcf",
        vcfgz=MDIR
        + "{sample}/align/{alnr}/{ddup}/snv/deep15/vcfs/{dvchrm}/{sample}.{alnr}.deep15.{dvchrm}.snv.sort.vcf.gz",
        vcftbi=MDIR
        + "{sample}/align/{alnr}/{ddup}/snv/deep15/vcfs/{dvchrm}/{sample}.{alnr}.deep15.{dvchrm}.snv.sort.vcf.gz.tbi",
    conda:
        config['deepvariant']['deep15_conda']
    log:
        MDIR
        + "{sample}/align/{alnr}/{ddup}/snv/deep15/vcfs/{dvchrm}/log/{sample}.{alnr}.deep15.{dvchrm}.snv.sort.vcf.gz.log",
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.{dvchrm}.deep15_sort_index_chunk_vcf.bench.tsv"
    resources:
        vcpu=4,
        threads=4,
...
rule deep15_concat_fofn:
    input:
        chunk_tbi=sorted(
            expand(
                MDIR
                + "{{sample}}/align/{{alnr}}/{{ddup}}/snv/deep15/vcfs/{dvchm}/{{sample}}.{{alnr}}.deep15.{dvchm}.snv.sort.vcf.gz.tbi",
    ...
    output:
        fin_fofn=MDIR
        + "{sample}/align/{alnr}/{ddup}/snv/deep15/{sample}.{alnr}.deep15.snv.concat.vcf.gz.fofn",
        tmp_fofn=MDIR        + "{sample}/align/{alnr}/{ddup}/snv/deep15/{sample}.{alnr}.deep15.snv.concat.vcf.gz.fofn.tmp",
        #gfin_fofn=MDIR+ "{sample}/align/{alnr}/{ddup}/snv/deep15/{sample}.{alnr}.deep15.snv.g.concat.vcf.gz.fofn",
        #gtmp_fofn=MDIR        + "{sample}/align/{alnr}/{ddup}/snv/deep15/{sample}.{alnr}.deep15.snv.g.concat.vcf.gz.fofn.tmp",
    threads: 2
    resources:
        vcpu=2,
        threads=2,
    ...
    conda:
        config['deepvariant']['deep15_conda']
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/deep15/log/{sample}.{alnr}.deep15.cocncat.fofn.log",
    shell:
        """
        ...
        (workflow/scripts/sort_concat_chrm_list.py {output.tmp_fofn} {wildcards.sample}.{wildcards.alnr}.deep15. {output.fin_fofn}) >> {log} 2>&1;

        """


rule deep15_concat_index_chunks:
    input:
        fofn=MDIR
        + "{sample}/align/{alnr}/{ddup}/snv/deep15/{sample}.{alnr}.deep15.snv.concat.vcf.gz.fofn",
        tmp_fofn=MDIR        + "{sample}/align/{alnr}/{ddup}/snv/deep15/{sample}.{alnr}.deep15.snv.concat.vcf.gz.fofn.tmp",
        #gfofn=MDIR
        #+ "{sample}/align/{alnr}/{ddup}/snv/deep15/{sample}.{alnr}.deep15.snv.g.concat.vcf.gz.fofn",
        #gtmp_fofn=MDIR        + "{sample}/align/{alnr}/{ddup}/snv/deep15/{sample}.{alnr}.deep15.snv.g.concat.vcf.gz.fofn.tmp",
    output:
        vcfgz=touch(
            MDIR + "{sample}/align/{alnr}/{ddup}/snv/deep15/{sample}.{alnr}.deep15.snv.sort.vcf.gz"
        ),
        vcfgztemp=temp(
            MDIR + "{sample}/align/{alnr}/{ddup}/snv/deep15/{sample}.{alnr}.deep15.snv.sort.temp.vcf.gz"
        ),
        vcfgztbi=touch(
            MDIR
            + "{sample}/align/{alnr}/{ddup}/snv/deep15/{sample}.{alnr}.deep15.snv.sort.vcf.gz.tbi"
        ),   #gvcf=touch(            MDIR + "{sample}/align/{alnr}/{ddup}/snv/deep15/{sample}.{alnr}.deep15.snv.g.sort.vcf"),        #gvcfgz=touch(            MDIR + "{sample}/align/{alnr}/{ddup}/snv/deep15/{sample}.{alnr}.deep15.snv.g.sort.vcf.gz"),
    threads: 4
    resources:
        vcpu=4,
        threads=4,
    ...
    log:
        MDIR
        + "{sample}/align/{alnr}/{ddup}/snv/deep15/log/{sample}.{alnr}.deep15.snv.merge.sort.gatherered.log",
    shell:
        """
...
rule clear_combined_deep15_vcf:  # TARGET:  clear combined deep vcf so the chunks can be re-evaluated if needed.
    input:
        vcf=expand(
            MDIR + "{sample}/align/{alnr}/{ddup}/snv/deep15/{sample}.{alnr}.deep15.snv.sort.vcf.gz",
            sample=SSAMPS,
    ...
    benchmark:
        MDIR + "benchmarks/clear_combined_deep15_vcf.bench.tsv"
    priority: 42
    conda:
        config['deepvariant']['deep15_conda']
    resources:
...
rule produce_deep15_vcf:  # DEPRECATED TARGET: use produce_deep15_snv_vcf
    input:
        vcftb=expand(
            MDIR
            + "{sample}/align/{alnr}/{ddup}/snv/deep15/{sample}.{alnr}.deep15.snv.sort.vcf.gz",
            sample=SSAMPS,
            alnr=ALIGNERS,
            ddup=DDUP,
        ),
        ...
        #gvcf=expand(
        #    MDIR
        #    + "{sample}/align/{alnr}/{ddup}/snv/deep15/{sample}.{alnr}.deep15.snv.g.sort.vcf.gzi",
        #    sample=SSAMPS,
        #    alnr=ALIGNERS,
        #    ddup=DDUP,
        #),
        #gvcftbi=expand(
        #    MDIR
        #    + "{sample}/align/{alnr}/{ddup}/snv/deep15/{sample}.{alnr}.deep15.snv.g.sort.vcf.gz.tbi",
        #    sample=SSAMPS,
        #    alnr=ALIGNERS,
        #    ddup=DDUP,
        #),
    ...
    benchmark:
        MDIR + "benchmarks/produce_deep15_vcf.bench.tsv"
    conda:
        config['deepvariant']['deep15_conda']
    params:
        cluster_sample=ret_sample,
...
rule prep_deep15_chunkdirs:
    input:
        b=MDIR + "{sample}/align/{alnr}/{ddup}/{sample}.{alnr}.{ddup}.cram",
        i=MDIR + "{sample}/align/{alnr}/{ddup}/{sample}.{alnr}.{ddup}.cram.crai",
    output:
        expand(
            MDIR + "{{sample}}/align/{{alnr}}/{{ddup}}/snv/deep15/vcfs/{dvchrm}/{{sample}}.ready",
            dvchrm=DEEPD_CHRMS,
        ),
    threads: 1
    log:
        MDIR + "{sample}/align/{alnr}/{ddup}/snv/deep15/log/{sample}.{alnr}.chunkdirs.log",
    benchmark:
        MDIR + "{sample}/benchmarks/{sample}.{alnr}.{ddup}.prep_deep15_chunkdirs.bench.tsv"
    shell:
        """
        ( echo {output}  ;
