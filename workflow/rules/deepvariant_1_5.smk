...
rule deepvariant_15:
    input:
        ...
        d=MDIR + "{sample}/align/{alnr}/snv/deep15/vcfs/{dvchrm}/{sample}.ready",
    output:
        vcf=temp(MDIR
        + "{sample}/align/{alnr}/snv/deep15/vcfs/{dvchrm}/{sample}.{alnr}.deep15.{dvchrm}.snv.vcf"),
        #gvcf=temp(MDIR + "{sample}/align/{alnr}/snv/deep15/vcfs/{dvchrm}/{sample}.{alnr}.deep15.{dvchrm}.snv.g.vcf"),
    log:
        MDIR
...
rule deep15_sort_index_chunk_vcf:
    ...
    output:
        vcfsort=MDIR
        + "{sample}/align/{alnr}/snv/deep15/vcfs/{dvchrm}/{sample}.{alnr}.deep15.{dvchrm}.snv.sort.vcf",
        vcfgz=MDIR
        + "{sample}/align/{alnr}/snv/deep15/vcfs/{dvchrm}/{sample}.{alnr}.deep15.{dvchrm}.snv.sort.vcf.gz",
        vcftbi=MDIR
...
rule deep15_concat_fofn:
    input:
        chunk_tbi=sorted(
            expand(
                MDIR
                + "{{sample}}/align/{{alnr}}/snv/deep15/vcfs/{dvchm}/{{sample}}.{{alnr}}.deep15.{dvchm}.snv.sort.vcf.gz.tbi",
    ...
    output:
        fin_fofn=MDIR
        + "{sample}/align/{alnr}/snv/deep15/{sample}.{alnr}.deep15.snv.concat.vcf.gz.fofn",
        tmp_fofn=MDIR        + "{sample}/align/{alnr}/snv/deep15/{sample}.{alnr}.deep15.snv.concat.vcf.gz.fofn.tmp",
        #gfin_fofn=MDIR+ "{sample}/align/{alnr}/snv/deep15/{sample}.{alnr}.deep15.snv.g.concat.vcf.gz.fofn",
        #gtmp_fofn=MDIR        + "{sample}/align/{alnr}/snv/deep15/{sample}.{alnr}.deep15.snv.g.concat.vcf.gz.fofn.tmp",
    threads: 2
    resources:
        vcpu=2,
        threads=2,
...
rule deep15_concat_index_chunks:
    input:
        ...
        tmp_fofn=MDIR        + "{sample}/align/{alnr}/snv/deep15/{sample}.{alnr}.deep15.snv.concat.vcf.gz.fofn.tmp",
        #gfofn=MDIR
        #+ "{sample}/align/{alnr}/snv/deep15/{sample}.{alnr}.deep15.snv.g.concat.vcf.gz.fofn",
        #gtmp_fofn=MDIR        + "{sample}/align/{alnr}/snv/deep15/{sample}.{alnr}.deep15.snv.g.concat.vcf.gz.fofn.tmp",
    output:
        vcfgz=touch(
            MDIR + "{sample}/align/{alnr}/snv/deep15/{sample}.{alnr}.deep15.snv.sort.vcf.gz"
        ),
        vcfgztemp=temp(
            MDIR + "{sample}/align/{alnr}/snv/deep15/{sample}.{alnr}.deep15.snv.sort.temp.vcf.gz"
        ),
        vcfgztbi=touch(
            MDIR
            + "{sample}/align/{alnr}/snv/deep15/{sample}.{alnr}.deep15.snv.sort.vcf.gz.tbi"
        ),   #gvcf=touch(            MDIR + "{sample}/align/{alnr}/snv/deep15/{sample}.{alnr}.deep15.snv.g.sort.vcf"),        #gvcfgz=touch(            MDIR + "{sample}/align/{alnr}/snv/deep15/{sample}.{alnr}.deep15.snv.g.sort.vcf.gz"),
    threads: 4
    resources:
        vcpu=4,
        threads=4,
...
rule produce_deep15_vcf:  # TARGET: deep variant vcf
    input:
        vcftb=expand(
            MDIR
            + "{sample}/align/{alnr}/snv/deep15/{sample}.{alnr}.deep15.snv.sort.vcf.gz",
            sample=SSAMPS,
            alnr=ALIGNERS,
        ),
        vcftbi=expand(
        ...
        #gvcf=expand(
        #    MDIR
        #    + "{sample}/align/{alnr}/snv/deep15/{sample}.{alnr}.deep15.snv.g.sort.vcf.gzi",
        #    sample=SSAMPS,
        #    alnr=ALIGNERS,
        #),
        #gvcftbi=expand(
        #    MDIR
        #    + "{sample}/align/{alnr}/snv/deep15/{sample}.{alnr}.deep15.snv.g.sort.vcf.gz.tbi",
        #    sample=SSAMPS,
        #    alnr=ALIGNERS,
        #),
    output:
...
rule prep_deep15_chunkdirs:
    ...
    output:
        expand(
            MDIR + "{{sample}}/align/{{alnr}}/snv/deep15/vcfs/{dvchrm}/{{sample}}.ready",
            dvchrm=DEEPD_CHRMS,
        ),
    threads: 1
    log:
        MDIR + "{sample}/align/{alnr}/snv/deep15/log/{sample}.{alnr}.chunkdirs.log",
    shell:
        """
        ( echo {output}  ;
