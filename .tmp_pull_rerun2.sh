...
for SESSION in test-sentdhipmr-3x test-sentdhuomr-3x test-sentdhupmr-3x; do
    ...
    tmux send-keys -t "$SESSION" "cd $DIR && source dyoainit && source bin/day_activate slurm hg38_broad && bash bin/day_run ${TARGETS[$SESSION]} -p -k -j 20 -T 1 -n 2>&1 | tee /tmp/${SESSION}_dryrun.log" Enter
